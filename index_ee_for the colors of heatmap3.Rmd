...
houdees=function(hoi, rt2, switch,mn,z,corr,date) {
  ...
  jpeg(paste("Heatmap of high ACMEs_mmaa",date, mn,".jpg"), width = width, height = height, quality = 100,pointsize = 14, res=300);# par( ps=ps)# par(cex.lab=90) 22 18
  ...
  cl.offset=20;cl.length=20;cl.cex = 1.05;pch.cex=1.05;pch=2;cl.pos = 'r'; #cl.offset=2;cl.length=5;cl.cex = 1.3;pch.cex=1.95;pch=14;
...
u3=all_all;  corr=FALSE;z='idir'; switch=1;
c1=c() #
c1= u3 #[u3[,'ADE'] < ADEMedian  & DV<ADEVar,] #& u3[,'z0.p']<ADEpval
# c1=c1[rev(order(c1[,'ACME'])),];
# c1= c1[sample(1:nrow(c1)), ];
...
jpeg(paste("Heatmap of high ACMEs_v8_pfas",date, mn,ga,".jpg"), width = width, height = height, quality = 100,pointsize = 14, res=300);# par( ps=ps)# par(cex.lab=90) 22 18
# col = brewer.pal(n = 9, name = "YlOrRd")
col=colorRampPalette(c('blue', 'white','orange'), alpha = TRUE)(150)
...
corrplot(resulta1, type = type, order = order,method=method, p.mat=p.mat.a1, tl.col = "black", #sum(COL2('RdBu')=="#FF7417")
         ...
         tl.srt = 90, diag = TRUE,col=col,is.corr = FALSE,col.lim=c(-0.4,0.4)) #only in age...0.001, -2,2 #rev(COL2('RdBu')[25:(length(COL2('RdBu'))-25)]) col.lim=c(min(resulta1),max(resulta1))
#non were significant in neg... but after mody yes!
